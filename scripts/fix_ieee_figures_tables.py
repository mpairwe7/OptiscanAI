...
def generate_fig1_architecture():
    ...
    for x, y, w, label, color in boxes_main:
        rect = mpatches.FancyBboxPatch(
            ...
            edgecolor='#333333', linewidth=1.0, alpha=0.85
        )
        ax.add_patch(rect)
    ...
    ax.text(5.0, 3.55, 'LangGraph Agentic Pipeline', ha='center',
            fontsize=7, fontweight='bold', color='#333333', style='italic')
    ...
    for x, y, label in agent_nodes:
        rect = mpatches.FancyBboxPatch(
            (x, y - 0.28), 1.1, 0.56,
            boxstyle='round,pad=0.08', facecolor=c_agent,
            edgecolor='#333333', linewidth=0.8, alpha=0.80
        )
        ax.add_patch(rect)
        ax.text(x + 0.55, y, label, ha='center', va='center',
    ...
    ax.annotate('', xy=(5.55, 3.15), xytext=(5.55, 4.1),
    ...
    for x, y, w, label, color in outputs:
        rect = mpatches.FancyBboxPatch(
            ...
            edgecolor='#333333', linewidth=0.8, alpha=0.75
        )
        ax.add_patch(rect)
        ax.text(x + w/2, y, label, ha='center', va='center',
    ...
    ax.annotate('', xy=(1.4, 1.75), xytext=(1.4, 2.5),
                arrowprops=dict(arrowstyle='->', color='#555555', lw=0.8))
    ax.annotate('', xy=(3.6, 1.75), xytext=(3.6, 2.5),
                arrowprops=dict(arrowstyle='->', color='#555555', lw=0.8))
    ax.annotate('', xy=(5.7, 1.75), xytext=(5.7, 2.5),
                arrowprops=dict(arrowstyle='->', color='#555555', lw=0.8))
    ax.annotate('', xy=(8.0, 1.75), xytext=(8.7, 2.5),
                arrowprops=dict(arrowstyle='->', color='#555555', lw=0.8))

    # Rejection path from gate
    ax.annotate('Reject', xy=(3.35, 3.95), xytext=(3.35, 4.15),
                fontsize=5.5, color=c_gate, fontweight='bold',
                ha='center', va='top',
                arrowprops=dict(arrowstyle='->', color=c_gate, lw=1.0))

    # Title area
    ax.text(5.0, 5.6, 'RetinalAI System Architecture', ha='center',
...
def generate_fig2_performance():
    ...
    bars1 = axes[0].bar(x - width/2, v1_vals, width, label='V1 Baseline',
                         color='#C55A5A', edgecolor='#333', linewidth=0.5, alpha=0.85)
    bars2 = axes[0].bar(x + width/2, v2_vals, width, label='V2 Precision Rescue',
                         color='#4472C4', edgecolor='#333', linewidth=0.5, alpha=0.85)
    ...
    bars3 = axes[1].bar(range(len(models)), auc_vals, color=colors,
...
def generate_fig3_gate():
    ...
    for i in range(len(layers) - 1):
        ...
        ax.annotate('', xy=(3.0, cy_to), xytext=(3.0, cy_from),
    ...
    rect_a = mpatches.FancyBboxPatch(
        (0.3, 0.6), 2.0, 0.7,
        boxstyle='round,pad=0.08', facecolor='#70AD47',
        edgecolor='#333', linewidth=0.8, alpha=0.85
    )
    ax.add_patch(rect_a)
    ax.text(1.3, 0.95, 'Accept\n(\u226570%)', ha='center', va='center',
            fontsize=6, fontweight='bold', color='white')

    # Reject
    rect_r = mpatches.FancyBboxPatch(
        (3.7, 0.6), 2.0, 0.7,
        boxstyle='round,pad=0.08', facecolor='#C00000',
        edgecolor='#333', linewidth=0.8, alpha=0.85
    )
    ax.add_patch(rect_r)
    ax.text(4.7, 0.95, 'Reject\n(+evidence)', ha='center', va='center',
            fontsize=6, fontweight='bold', color='white')

    # Arrows to accept/reject
    ax.annotate('', xy=(1.3, 1.35), xytext=(2.2, 1.75),
                arrowprops=dict(arrowstyle='->', color='#70AD47', lw=1.2))
    ax.annotate('', xy=(4.7, 1.35), xytext=(3.8, 1.75),
...
def insert_figure_after_para(doc, para_index, image_path, caption_text,
                              width_inches=3.5):
    ...
    cap_para = parse_xml(f'<w:p {nsdecls("w")}><w:pPr>'
                         f'<w:pStyle w:val="FigCaption"/>'
                         f'</w:pPr></w:p>')
...
def add_figure_references(doc):
    ...
    for p in doc.paragraphs:
        ...
        if ('six-node directed acyclic graph' in old_text
                and 'Fig.' not in old_text):
        ...
        if ('Table I summarizes' in old_text and 'Fig.' not in old_text):
            new_text = old_text.replace(
                'Table I summarizes the improvement',
                'Table I and Fig. 3 summarize the improvement'
            )
            if new_text != old_text:
                _replace_para_text(p, new_text)
                continue
        ...
        if ('four-layer image quality gate' in old_text
                and 'Fig.' not in old_text):
            new_text = old_text.replace(
...
def main():
    ...
    target_idx = None
    for i, p in enumerate(doc.paragraphs):
        if ('network outages in Uganda' in p.text
                or 'deterministic rule-based logic' in p.text):
            target_idx = i
    if target_idx:
        add_figure_properly(doc, target_idx, fig1_path,
    ...
    for i, p in enumerate(doc.paragraphs):
        if 'prospective validation is a necessary' in p.text.lower():
            target_idx = i
    if target_idx:
        add_figure_properly(doc, target_idx, fig2_path,
    ...
    for i, p in enumerate(doc.paragraphs):
        if 'false acceptance rate below 1.5%' in p.text:
            target_idx = i
    if target_idx:
        add_figure_properly(doc, target_idx, fig3_path,
    ...
    body_text = ' '.join(p.text for p in doc2.paragraphs
                         if p.style.name in ('PARA', 'PARA_Indent'))
