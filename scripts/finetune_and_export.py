...
def main():
    ...
    train_ds = PreCachedDataset(
        to_df(dm.train_dataset), img_dirs, dm.disease_columns, train_tfm, img_size=IMG_SIZE
    )
    val_ds = PreCachedDataset(
        to_df(dm.val_dataset), img_dirs, dm.disease_columns, val_tfm, img_size=IMG_SIZE
    )
    train_ld = DataLoader(
        train_ds,
    ...
    for ep in range(FINETUNE_EPOCHS):
        ...
        log.info(
            f"  E{ep+1}/{FINETUNE_EPOCHS} | L:{tloss/max(steps,1):.4f}/{vloss/max(vs,1):.4f} | F1:{f1:.4f} AUC:{auc:.4f} P:{prec:.4f} R:{rec:.4f} Acc:{acc:.4f} lr:{lr:.1e}"
        )
    ...
    log.info(
        f"  Fixed threshold (0.5):  F1={fixed_metrics['f1_macro']:.4f} AUC={fixed_metrics.get('auc_roc',0):.4f} P={fixed_metrics.get('precision_macro',0):.4f} R={fixed_metrics.get('recall_macro',0):.4f}"
    )
    log.info(
        f"  Optimized thresholds:   F1={opt_metrics['f1_macro']:.4f} AUC={opt_metrics.get('auc_roc',0):.4f} P={opt_metrics.get('precision_macro',0):.4f} R={opt_metrics.get('recall_macro',0):.4f}"
    )
    log.info(
        f"  Threshold range: [{optimal_thresholds.min():.3f}, {optimal_thresholds.max():.3f}] mean={optimal_thresholds.mean():.3f}"
    )
    ...
    try:
        ...
        log.info(
            f"  ONNX: {onnx_path} | valid={val['valid']} | max_diff={val.get('max_diff',0):.6f}"
        )
    ...
    try:
        ...
        ts_path = TorchScriptExporter.trace(
            export_model, export_dir / "sgt_model_traced.pt", device="cpu"
        )
    ...
    print(
        f"  Thresholds:   [{optimal_thresholds.min():.3f}, {optimal_thresholds.max():.3f}] mean={optimal_thresholds.mean():.3f}"
    )
