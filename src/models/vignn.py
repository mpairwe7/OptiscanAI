...
class ClinicalKnowledgeGraph:
    ...
    def __init__(self, disease_names):
        ...
        self.uganda_prevalence = {
            code: value for code, value in raw_prevalence.items() if resolve(code)
        }
        ...
        self.visual_features = {
            code: feats for code, feats in raw_visual_features.items() if resolve(code)
        }
    ...
    def calculate_composite_risk_score(self, predictions, age=None, comorbidities=None):
        ...
        for disease, pred_conf in predictions.items():
            ...
            disease_weighted_risk = disease_risk * (
                0.3 + severity_weight + prevalence_weight + age_weight + comorbidity_weight
            )
            weights[disease] = {
    ...
    def summarize_priority_condition(self, condition_key: str) -> Optional[Dict[str, Any]]:
        ...
        if dataset_code and dataset_code in self.disease_names:
            return {
                ...
                "category_membership": [
                    cat for cat, diseases in self.categories.items() if dataset_code in diseases
                ],
            }
        return {
            "condition_key": condition_key,
...
class SparseTopKAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        attn_output = (
            attn_output.transpose(1, 2).contiguous().view(batch_size, seq_len_q, self.embed_dim)
        )
...
class MultiResolutionEncoder(nn.Module):
    ...
    def __init__(self, backbone_name="vit_small_patch16_224", output_dim=384, img_size=224):
        ...
        self.resolutions = (
            [img_size] if self.fast_mode else [img_size, int(img_size * 0.71), int(img_size * 0.57)]
        )
...
class ViGNN(nn.Module):
    ...
    def __init__(
        ...
        if clinical_knowledge_graph is None:
            raise ValueError(
                "ViGNN requires clinical_knowledge_graph parameter (ClinicalKnowledgeGraph instance)"
            )
    ...
    def forward(self, x):
        ...
        patch_disease_concat = torch.cat(
            [patch_mean.expand(-1, self.num_classes, -1), disease_proto], dim=-1
        )
        ...
        disease_out, _ = self.disease_attention(disease_query, graph_embeds, graph_embeds)
