...
def upgrade() -> None:
    ...
    sa.Enum(
        "owner", "admin", "clinician", "viewer", name="membership_role",
    ).create(op.get_bind(), checkfirst=True)
    sa.Enum(
        "pending", "active", "revoked", name="membership_status",
    ).create(op.get_bind(), checkfirst=True)
    sa.Enum(
    ...
    sa.Enum(
        "stripe", "mtn", "airtel", "flutterwave", "manual", name="payment_provider",
    ).create(op.get_bind(), checkfirst=True)
    sa.Enum(
    ...
    sa.Enum(
        "scan", "explain_gradcam", "explain_lime", "explain_shap",
        "explain_ig", "explain_eli5", "clinical_reasoning", "audit_export",
    ...
    op.create_index("ix_subscription_status_period_end", "subscriptions", ["status", "current_period_end"])
    ...
    op.create_table(
        ...
        sa.Column("invoice_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("invoices.id", ondelete="CASCADE"), nullable=True),
        sa.Column("provider", payment_provider, nullable=False),
        ...
        sa.Column("amount_cents", sa.Integer, nullable=False),
        sa.Column("currency", sa.String(3), nullable=False, server_default="USD"),
        sa.Column("status", payment_intent_status, nullable=False, server_default="requires_action"),
    ...
    op.create_index("ix_usage_org_type_occurred", "usage_events",
...
def downgrade() -> None:
    ...
    for enum_name in [
        "usage_event_type", "payment_intent_status", "invoice_status",
        "payment_provider", "billing_cycle", "subscription_status",
        "membership_status", "membership_role",
    ]:
