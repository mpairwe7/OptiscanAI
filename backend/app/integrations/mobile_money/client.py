...
class MobileMoneyClient:
    ...
    async def _mtn_request(self, tx_id: str, phone: str, amount: int, currency: str, reason: str) -> PaymentRequest:
        """MTN MoMo Collections API v1."""
        base_url = "https://sandbox.momodeveloper.mtn.com" if self._mtn_env == "sandbox" else "https://momodeveloper.mtn.com"
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{base_url}/collection/v1_0/requesttopay", headers=headers, json=payload) as resp:
                    if resp.status == 202:
                        return PaymentRequest(transaction_id=tx_id, phone=phone, amount=amount, currency=currency, reason=reason, provider="mtn", status="pending")
    ...
    async def _airtel_request(self, tx_id: str, phone: str, amount: int, currency: str, reason: str) -> PaymentRequest:
        ...
        return PaymentRequest(transaction_id=tx_id, phone=phone, amount=amount, currency=currency, reason=reason, provider="airtel", status="pending")

    async def check_payment_status(self, transaction_id: str, provider: str = "mtn") -> PaymentStatus:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        headers={"Ocp-Apim-Subscription-Key": self._mtn_sub_key, "X-Target-Environment": self._mtn_env},
                    ) as resp:
                        data = await resp.json()
                        return PaymentStatus(transaction_id=transaction_id, status=data.get("status", "unknown").lower(), provider="mtn")
            except Exception as e:
