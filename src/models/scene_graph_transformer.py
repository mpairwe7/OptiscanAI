...
class SceneGraphTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.relation_attn = SparseTopKAttention(
            hidden_dim, num_heads=num_heads, dropout=dropout, top_k=8
        )
    ...
    def forward(self, x):
        ...
        region_indices = torch.linspace(
            0, num_patches - 1, self.num_regions, dtype=torch.long, device=x.device
        )
