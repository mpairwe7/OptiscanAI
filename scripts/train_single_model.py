...
def train_fold(model, train_ld, val_ld, criterion, device, fold, mname):
    ...
    log.info(
        f"  [{mname}] F{fold+1} DiffLR backbone={BACKBONE_LR:.0e}({len(backbone_params)}p) head={HEAD_LR:.0e}({len(head_params)}p)"
    )
    ...
    for ep in range(MAX_EPOCHS):
        ...
        prec, rec, acc = (
            vm.get("precision_macro", 0),
            vm.get("recall_macro", 0),
            vm.get("accuracy_sample", 0),
        )
        lr_now = opt.param_groups[1]["lr"]
        log.info(
            f"  [{mname}] F{fold+1} E{ep+1}/{MAX_EPOCHS} | L:{tloss/max(steps,1):.4f}/{vloss/max(vs,1):.4f} | F1:{f1:.4f} AUC:{auc:.4f} P:{prec:.4f} R:{rec:.4f} Acc:{acc:.4f} lr:{lr_now:.1e}"
        )
        if f1 > best["f1_macro"]:
...
def main():
    ...
    parser.add_argument(
        "--model",
        required=True,
        choices=["vignn", "graphclip", "visual_language_gnn", "scene_graph_transformer"],
    )
